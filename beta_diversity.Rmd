...
Chargez la matrice de présence-absence et la localisation des sites en utilisant la commande `readRDS()`. Transformez le tableau de localisation des sites en objet spatial de type `sf` avec la commande `st_as_sf()`, en spécifiant bien quelles sont les colonnes contenant les coordonnées avec l'argument `coords`
